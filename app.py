...
current_ids = ['spotify:playlist:28mI8s2FBTx0pzWfQ7dTwA', '7s5PGUSUudnrdcueQuxWW0',
               'spotify:playlist:6SeTiLgmsdVbQ4MjOnIJb6']
# data = pd.DataFrame()
...
checklist = html.Div([
    ...
    html.Div([dbc.Button("Add", id="add", style={'margin': 'auto'}),
              dbc.Button("Remove", id="clear-done", style={'margin': 'auto'})],
             style={'display': 'flex', 'padding-top': '10px', 'padding-bottom': '5px'}),
...
row2 = dbc.Row(
    [dbc.Col([html.H3("Generate a New Playlist"), row2_form], width=3), dbc.Col([dcc.Graph(id='table')], width=9)])
...
content = html.Div(
    [dbc.Row([dbc.Col(dcc.Graph(id="playlist_graph"), width=6), dbc.Col(dcc.Graph(id="playlist_graph2"), width=6)]),
     row2], style=CONTENT_STYLE)
...
def store_playlist_id_mapping(df, df2):
    ...
    if df is None:
        return dict(), dict()
    df = pd.read_json(df)
    dictionary = (pd.Series(df['playlist_id'].values, index=df['playlist']).to_dict(),
                  pd.Series(df['playlist'].values, index=df['playlist_id']).to_dict())
    return dictionary
...
def edit_list(dataframe, playlist_ids, mapping):
    ...
    new_list = [
        html.Div([
            ...
            html.Div((lambda x: pl_name_dict[x] if x in pl_name_dict.keys() else x)(text), id={"index": i},
